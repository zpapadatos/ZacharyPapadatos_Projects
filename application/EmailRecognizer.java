...
public class EmailRecognizer {
	...
	public static String checkForValidEmail(String input) {
		...
		while (running) {
			...
			case 2: 
				// State 2 deals with a character after a special character is included in the name.
				
				// A-Z, a-z, 0-9, . -> State 1
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						(currentChar >= 'a' && currentChar <= 'z' ) || 
						(currentChar >= '0' && currentChar <= '9')||
						(currentChar == '.')) {	// Check for 0-9
					nextState = 1;
		...
		case 0:
			...
			EmailRecognizerErrorMessage += "A Email must start with A-Z, or a-z. Please try again\n";
		...
		case 1:
			...
			if (userNameSize < 4) {
				// Email is too small
				EmailRecognizerErrorMessage += "A Email must have at least 4 characters.\n";
				return EmailRecognizerErrorMessage;
			}
			else if (userNameSize > 36) {
				// Email is too long
				EmailRecognizerErrorMessage += 
					"A email must have no more than 36 character.\n";
				return EmailRecognizerErrorMessage;
			}
			else if (currentCharNdx < input.length()) {
				// There are characters remaining in the input, so the input is not valid
				EmailRecognizerErrorMessage += 
					"A email character may only contain the characters A-Z, a-z, @,. \n";
				return EmailRecognizerErrorMessage;
			}
			else {
					// Email is valid
		...
		case 2:
			...
			EmailRecognizerErrorMessage +=
				"A UserName character after a special character must be A-Z, a-z,.\n";
			return EmailRecognizerErrorMessage;
